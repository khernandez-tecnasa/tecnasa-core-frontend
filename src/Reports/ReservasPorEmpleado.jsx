...
export default function ReservasPorEmpleado() {
  ...
  const maxReservas = useMemo(() => Math.max(...filtered.map((r) => Number(r.total_reservas ?? 0)), 1), [filtered]);
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-10 space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-black tracking-tight leading-none">Reservas por Empleado</h1>
              <p className="text-muted-foreground text-xs font-medium mt-0.5">{loading ? "Cargando..." : `${filtered.length} empleado${filtered.length !== 1 ? "s" : ""}`}</p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-3xl p-4 space-y-3 shadow-sm">
        <div className="relative group">
          ...
          <input type="text" placeholder="Buscar por nombre o puesto..." defaultValue={query} onChange={(e) => onChangeQuery(e.target.value)}
        ...
        <div className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-3xl shadow-sm overflow-hidden">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/60 bg-muted/20">
                    {["#", "Empleado", "Puesto", "Total", "Finaliz.", "Cancelad.", "En Uso", "Pendientes"].map((h, idx) => (
          ...
          {pageItems.length > 0 && (
            <div className="px-5 py-3.5 border-t border-border/40 flex items-center justify-between gap-4 bg-muted/10">
              <p className="text-xs text-muted-foreground font-medium">Página {pageSafe} de {totalPages} · {filtered.length} empleado{filtered.length !== 1 ? "s" : ""}</p>
              <PaginationLite page={pageSafe} count={totalPages} onChange={setPage} size="sm" />
            </div>
          )}
        </div>
      )}
      <ExportDialog open={openExport} onClose={() => setOpenExport(false)} rows={filtered} columns={columnsExport} defaultTitle="Reservas por Empleado" defaultFilenameBase="reservas_empleado" />
    </div>
  );
}
