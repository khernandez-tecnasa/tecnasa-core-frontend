    ...
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 md:py-10 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-black tracking-tight leading-none">Viáticos por Estado</h1>
              <p className="text-muted-foreground text-xs font-medium mt-0.5">
                {loading ? "Cargando..." : `${totalCount} viático${totalCount !== 1 ? "s" : ""} · ${fmtCurrency(totalMonto)} en total`}
              </p>
            </div>
          </div>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center gap-4 py-24">
          ...
          <p className="text-sm text-muted-foreground font-medium">Cargando viáticos...</p>
        </div>
      ) : err ? (
        ...
        <div className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-3xl flex flex-col items-center justify-center gap-4 py-20">
          ...
          <p className="font-bold text-sm">Sin viáticos</p>
              ...
              return (
                <div key={r.estado} className={`${s.bg} ${s.border} border rounded-3xl p-5 space-y-3`}>
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-muted-foreground font-medium">viáticos</p>
                  </div>
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground"><Hash size={11} />{pctCount}% del conteo</div>
                  </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/60 bg-muted/20">
                    {["Estado", "Viáticos", "Monto Total", "% del Total"].map((h) => (
      ...
      <ExportDialog open={openExport} onClose={() => setOpenExport(false)} rows={raw} columns={columnsExport} defaultTitle="Viáticos por Estado" defaultFilenameBase="viaticos_estado" />
    </div>
  );
}
