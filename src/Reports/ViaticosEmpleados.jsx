...
import {
  ArrowLeft, Download, TrendingUp, Loader2, AlertCircle, Calendar, X, Search, User, Route,
} from "lucide-react";
    ...
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 md:py-10 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-black tracking-tight leading-none">{t("reports.viaticosEmpleados.title")}</h1>
              <p className="text-muted-foreground text-xs font-medium mt-0.5">{loading ? t("reports.viaticosEmpleados.loading") : t("reports.viaticosEmpleados.count", { count: raw.length })}</p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-3xl p-4 shadow-sm space-y-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={t("reports.viaticosEmpleados.search_placeholder")}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center gap-4 py-24">
          ...
          <p className="text-sm text-muted-foreground font-medium">{t("reports.viaticosEmpleados.loading")}</p>
        </div>
      ) : err ? (
        ...
        <div className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-3xl flex flex-col items-center justify-center gap-4 py-20">
          ...
          <p className="font-bold text-sm">{t("reports.viaticosEmpleados.empty_title")}</p>
          <p className="text-xs text-muted-foreground">{q ? t("reports.viaticosEmpleados.empty_text_search") : t("reports.viaticosEmpleados.empty_period")}</p>
        </div>
      ) : (
        <>
          {/* Desktop Table */}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/60 bg-muted/20">
                    {[
                      "#",
                      t("reports.viaticosEmpleados.col_employee"),
                      t("reports.viaticosEmpleados.col_position"),
                      t("reports.viaticosEmpleados.col_viaticos"),
                      t("reports.viaticosEmpleados.col_amount"),
                      t("reports.viaticosEmpleados.col_km"),
                    ].map((h) => (
              ...
              return (
                <div key={r.nombre_empleado} className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-2xl p-4 flex items-center gap-3">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-[10px] text-muted-foreground">{r.total_viaticos} {t("reports.viaticosEmpleados.mobile_viaticos")}</p>
                  </div>
                </div>
              );
            })}
          ...
          {filtered.length < raw.length && (
            <p className="text-center text-xs text-muted-foreground">{t("reports.viaticosEmpleados.showing", { shown: filtered.length, total: raw.length })}</p>
          )}
        </>
      )}

      <ExportDialog open={openExport} onClose={() => setOpenExport(false)} rows={filtered} columns={columnsExport} defaultTitle={t("reports.viaticosEmpleados.export_title")} defaultFilenameBase="viaticos_empleados" />
    </div>
  );
}
