...
const TIPO_CONFIG = {
  DESAYUNO:   { icon: Utensils,   bg: "bg-amber-50 dark:bg-amber-900/20",   border: "border-amber-200 dark:border-amber-800/40",   text: "text-amber-700 dark:text-amber-400",   dot: "bg-amber-500" },
  ALMUERZO:   { icon: Utensils,   bg: "bg-orange-50 dark:bg-orange-900/20",  border: "border-orange-200 dark:border-orange-800/40",  text: "text-orange-700 dark:text-orange-400",  dot: "bg-orange-500" },
  CENA:       { icon: Moon,       bg: "bg-indigo-50 dark:bg-indigo-900/20",  border: "border-indigo-200 dark:border-indigo-800/40",  text: "text-indigo-700 dark:text-indigo-400",  dot: "bg-indigo-500" },
  HOSPEDAJE:  { icon: BedDouble,  bg: "bg-violet-50 dark:bg-violet-900/20",  border: "border-violet-200 dark:border-violet-800/40",  text: "text-violet-700 dark:text-violet-400",  dot: "bg-violet-500" },
  COMBUSTIBLE:{ icon: Fuel,       bg: "bg-red-50 dark:bg-red-900/20",        border: "border-red-200 dark:border-red-800/40",        text: "text-red-700 dark:text-red-400",        dot: "bg-red-500" },
  PEAJE:      { icon: CreditCard, bg: "bg-slate-50 dark:bg-slate-900/30",    border: "border-slate-200 dark:border-slate-700/40",    text: "text-slate-700 dark:text-slate-400",    dot: "bg-slate-500" },
  IMPREVISTO: { icon: Zap,        bg: "bg-rose-50 dark:bg-rose-900/20",      border: "border-rose-200 dark:border-rose-800/40",      text: "text-rose-700 dark:text-rose-400",      dot: "bg-rose-500" },
};
const DEFAULT_TIPO = { icon: Hash, bg: "bg-muted/30", border: "border-border/50", text: "text-muted-foreground", dot: "bg-muted-foreground" };
    ...
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 md:py-10 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-black tracking-tight leading-none">{t("reports.viaticosType.title")}</h1>
              <p className="text-muted-foreground text-xs font-medium mt-0.5">
                {loading ? t("reports.viaticosType.loading") : t("reports.viaticosType.count", { count: totalCantidad, amount: fmtCurrency(totalMonto) })}
              </p>
            </div>
          </div>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center gap-4 py-24">
          ...
          <p className="text-sm text-muted-foreground font-medium">{t("reports.viaticosType.loading")}</p>
        </div>
      ) : err ? (
        ...
        <div className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-3xl flex flex-col items-center justify-center gap-4 py-20">
          ...
          <p className="font-bold text-sm">{t("reports.viaticosType.empty_title")}</p>
          <p className="text-xs text-muted-foreground">{t("reports.viaticosType.empty_period")}</p>
        </div>
      ) : (
        <>
          {/* Tarjetas de tipo */}
                ...
                <div key={r.tipo} className={`${cfg.bg} ${cfg.border} border rounded-3xl p-5 space-y-3`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs font-black uppercase tracking-wide ${cfg.text}`}>{r.tipo}</span>
                    </div>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{t("reports.viaticosType.card_records", { count: r.cantidad })}</p>
                  </div>
          ...
          {totalMonto > 0 && (
            <div className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-3xl p-5 shadow-sm space-y-3">
              <p className="text-xs font-black uppercase tracking-widest text-muted-foreground/60">{t("reports.viaticosType.chart_title")}</p>
                ...
                {raw.map((r) => {
                  ...
                  return (
                    <div key={r.tipo} className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs text-muted-foreground">{r.tipo} <span className="font-bold text-foreground">{fmtCurrency(r.monto_total)}</span></span>
                    </div>
                  );
                })}
              </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/60 bg-muted/20">
                    {[
                      t("reports.viaticosType.col_type"),
                      t("reports.viaticosType.col_quantity"),
                      t("reports.viaticosType.col_amount"),
                      t("reports.viaticosType.col_dist"),
                    ].map((h) => (
                      ...
                      <tr key={r.tipo} className="border-b border-border/30 last:border-0 hover:bg-muted/20 transition-colors">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-sm font-bold ${cfg.text}`}>{r.tipo}</span>
                          </div>
                        </td>
                        <td className="px-5 py-4">
      ...
      <ExportDialog open={openExport} onClose={() => setOpenExport(false)} rows={raw} columns={columnsExport} defaultTitle={t("reports.viaticosType.export_title")} defaultFilenameBase="viaticos_tipo" />
    </div>
  );
}
