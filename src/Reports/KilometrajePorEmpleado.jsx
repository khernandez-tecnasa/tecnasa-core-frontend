// src/Reports/KilometrajePorEmpleado.jsx
  ...
  return (
    <div
      className={`w-9 h-9 rounded-xl flex items-center justify-center text-xs font-black shrink-0 transition-all duration-200 ${
        isTop3
          ? "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 ring-1 ring-amber-200 dark:ring-amber-800/40"
...
// ── Barra de km proporcional ───────────────────────────────────────────────────
function KmBar({ value, max }) {
  const raw = Number(value);
  const num = Number.isFinite(raw) ? raw : 0;
  const pct = max > 0 ? Math.round((num / max) * 100) : 0;
  return (
    <div className="flex items-center gap-2">
      <span className="text-sm font-black text-amber-600 dark:text-amber-400 tabular-nums text-right shrink-0 min-w-[72px]">
        {fmtKm(num)}
      </span>
...
export default function KilometrajePorEmpleado() {
  ...
  const maxKm = useMemo(
    () => Math.max(...filtered.map((r) => Number(r.kilometraje_total_recorrido) || 0), 1),
    [filtered]
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-2xl bg-amber-500/10 dark:bg-amber-500/15 ring-1 ring-amber-500/20 dark:ring-amber-500/30 shadow-sm shadow-amber-500/10 shrink-0">
              <Route size={20} className="text-amber-600 dark:text-amber-400" />
            </div>
            <div>
                      ...
                      <tr
                        ...
                        <td className="px-5 py-4">
                          <div className="flex justify-end">
                            <KmBar value={r.kilometraje_total_recorrido} max={maxKm} />
                          </div>
                        </td>
                      </tr>
                    );
