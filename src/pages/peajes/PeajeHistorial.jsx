import { useEffect, useState } from "react";
import { useNavigate, useParams, useLocation } from "react-router-dom";
import { ArrowLeft, Milestone, MapPin, Loader2 } from "lucide-react";

import AuditTimeline from "@/components/ui/AuditTimeline";
import { getPeajes } from "@/services/peajes.service";

export default function PeajeHistorial() {
  const { id }   = useParams();
  const navigate = useNavigate();
  const location = useLocation();

  const [peaje, setPeaje]     = useState(location.state?.peaje ?? null);
  const [loading, setLoading] = useState(!location.state?.peaje);

  useEffect(() => {
    if (peaje) return;
    let cancelled = false;
    getPeajes()
      .then((raw) => {
        if (cancelled) return;
        const list = Array.isArray(raw) ? raw : [];
        const found = list.find((p) => String(p.id) === String(id));
        setPeaje(found ?? null);
      })
      .catch(() => setPeaje(null))
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [id, peaje]);

  return (
    <div className="max-w-3xl mx-auto px-4 md:px-8 py-6 md:py-10 space-y-8 animate-in fade-in duration-500">

      {/* ── Header ── */}
      <div className="flex items-start gap-4">
        <button
          onClick={() => navigate("/admin/peajes")}
          className="mt-0.5 p-2 hover:bg-muted dark:hover:bg-slate-800 rounded-xl transition-colors text-muted-foreground hover:text-foreground shrink-0">
          <ArrowLeft size={18} />
        </button>
        <div className="flex items-center gap-3.5">
          <div className="p-2.5 rounded-2xl bg-primary/10 dark:bg-primary/15 ring-1 ring-primary/20 dark:ring-primary/30 shadow-sm shadow-primary/10 shrink-0">
            <Milestone size={22} className="text-primary" />
          </div>
          <div>
            <h1 className="text-2xl md:text-3xl font-black tracking-tight leading-none">
              Historial del peaje
            </h1>
            <p className="text-muted-foreground text-xs md:text-sm font-medium mt-0.5">
              Registro de cambios y eventos
            </p>
          </div>
        </div>
      </div>

      {/* ── Info card ── */}
      {loading ? (
        <div className="flex items-center justify-center gap-3 py-10 text-muted-foreground">
          <Loader2 size={18} className="animate-spin" />
          <span className="text-sm">Cargando...</span>
        </div>
      ) : peaje ? (
        <div className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-3xl shadow-sm p-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 shrink-0 rounded-2xl bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center">
              <Milestone size={22} className="text-amber-600 dark:text-amber-500" />
            </div>
            <div className="flex-1 min-w-0">
              <span className="text-xl font-black tracking-tight text-gray-900 dark:text-gray-100">
                {peaje.nombre || "—"}
              </span>
              {(peaje.latitud != null || peaje.longitud != null) && (
                <div className="flex items-center gap-1 mt-1 text-xs font-mono text-muted-foreground/70">
                  <MapPin size={11} className="text-primary shrink-0" />
                  {Number(peaje.latitud || 0).toFixed(5)},{" "}
                  {Number(peaje.longitud || 0).toFixed(5)}
                </div>
              )}
            </div>
          </div>
        </div>
      ) : (
        <div className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-3xl shadow-sm p-6 text-center text-muted-foreground text-sm">
          Peaje no encontrado.
        </div>
      )}

      {/* ── Audit timeline ── */}
      <div className="bg-card dark:bg-slate-900/40 border border-border/60 rounded-3xl shadow-sm p-6">
        <AuditTimeline entidad="peaje" entidadId={Number(id)} />
      </div>

    </div>
  );
}
